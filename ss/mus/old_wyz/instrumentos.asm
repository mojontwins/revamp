
; Tabla de instrumentos
TABLA_PAUTAS: DW PAUTA_0,PAUTA_1,PAUTA_2,PAUTA_3,PAUTA_4,PAUTA_5,PAUTA_6

; Tabla de efectos
TABLA_SONIDOS: DW SONIDO0,SONIDO1,SONIDO2,SONIDO3

;Pautas (instrumentos)
;Instrumento 'Piano'
PAUTA_0:	DB	47,0,15,0,10,0,9,0,8,0,8,0,7,0,7,0,7,0,6,0,6,0,6,0,6,0,0,0,129
;Instrumento 'PICC'
PAUTA_1:	DB	76,0,13,0,12,0,11,0,10,0,8,0,7,0,5,0,6,0,129
;Instrumento 'Flauta vol bajo'
PAUTA_2:	DB	14,0,12,0,10,0,9,0,9,0,8,0,8,0,7,0,7,0,8,0,8,0,7,0,7,0,6,0,6,0,7,0,7,0,136
;Instrumento 'Flauta'
PAUTA_3:	DB	10,0,12,0,13,0,13,0,13,0,12,0,11,0,11,0,11,0,10,0,10,0,10,0,10,0,10,0,9,0,9,0,9,0,9,0,9,0,138
;Instrumento 'Picc vol 2'
PAUTA_4:	DB	78,0,13,0,11,0,3,0,9,0,8,0,2,0,7,0,13,0,6,0,1,0,4,0,1,0,5,0,1,0,134
;Instrumento 'Eco'
PAUTA_5:	DB	6,0,8,0,9,0,8,0,7,0,6,0,129
;Instrumento 'Nuevo'
PAUTA_6:	DB	13,0,11,0,10,0,9,0,8,0,9,0,10,0,12,0,12,0,12,0,10,0,8,0,8,0,6,0,7,0,139

;Efectos
;Efecto 'bass drum'
SONIDO0:	DB	209,63,0,209,78,0,69,109,0,255
;Efecto 'drum'
SONIDO1:	DB	69,47,0,232,44,5,0,57,2,255
;Efecto 'hithat'
SONIDO2:	DB	0,11,1,0,6,1,255
;Efecto 'bass drum vol 2'
SONIDO3:	DB	186,58,0,0,102,0,162,131,0,255

;Frecuencias para las notas
DATOS_NOTAS: DW 0,0
DW 1711,1614,1524,1438,1358,1281,1210,1142,1078,1017
DW 960,906,855,807,762,719,679,641,605,571
DW 539,509,480,453,428,404,381,360,339,320
DW 302,285,269,254,240,227,214,202,190,180
DW 170,160,151,143,135,127,120,113,107,101
DW 95,90,85,80,76,71,67,64,60,57
